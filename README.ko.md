...
- **툴바 기능**: 텍스트 서식, 목록, 미디어 삽입 지원. 다단계 실행 취소/다시 실행(Undo/Redo) 지원.
- **게시글 템플릿**: 마크다운 템플릿을 저장하고 재사용하여 일관된 글쓰기 워크플로우 지원
- **미디어 삽입**: 이미지 붙여넣기 및 드래그 앤 드롭 지원, GIPHY 스티커 선택기를 통한 풍성한 표현
- **이미지 정렬 및 크기 조절 문법**: 커스텀 마크다운 `![alt](url?align=center&width=W&height=H)`를 통한 이미지 정렬(좌/우/중앙) 및 너비/높이 정밀 지정
- **커버 이미지** 업로드 지원
...
- **반응형 에디터**: 모바일에서 제목/카테고리/태그 행이 자동으로 분리되는 최적화된 레이아웃
...
- **시맨틱 테마 토큰**: CSS 변수 기반 색상 시스템 (primary, destructive, success) + 완벽한 다크 모드 지원
- **웹 성능 최적화(Core Web Vitals)**: 첫 번째 이미지 우선 로딩 및 지연 로딩을 통한 LCP 개선
- **헤더 & 푸터 커스터마이징**: 배경색, 배경 이미지, 높이 조절
- **반응형 디자인**: 모바일 최적화 + 스크롤 시 헤더 자동 축소
- **글래스모피즘** 효과 (헤더/푸터)
- **인쇄 최적화**: 인쇄 시 불필요한 UI를 가리고, 다크모드에서도 잉크 낭비 없이 텍스트와 본문 내용만 깔끔하게 출력되도록 완벽히 지원합니다.
...
- **실시간 대시보드**: 최근 24시간 동안의 순 방문자 수와 총 페이지 뷰를 실시간으로 확인합니다.
- **상세 로그**: IP, 국가, OS, 브라우저, 유입 경로(Referer)를 포함한 최근 24시간 방문자 기록을 제공합니다.
...
│           └── ui/__tests__/ # 컴포넌트 단위 테스트 (Vitest + Testing Library)
├── server/           # 백엔드 (Hono)
│   └── src/
