...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    catch (err) { setError(err instanceof Error ? err.message : t("request_failed")); }
      ...
      <Card className="w-full max-w-md">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div><label className="mb-1 block text-sm font-medium text-[var(--color-text)]">{t("login_email_placeholder")}</label><Input type="email" placeholder="admin@example.com" value={email} onChange={(e) => setEmail(e.target.value)} required autoFocus /></div>
            <div><label className="mb-1 block text-sm font-medium text-[var(--color-text)]">{t("login_password_placeholder")}</label><Input type="password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} required /></div>
