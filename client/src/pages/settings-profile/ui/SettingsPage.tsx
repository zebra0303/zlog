...
export default function SettingsPage() {
  ...
  const handleAboutPaste = useCallback(
  ...
  const handleAboutDrop = useCallback(
  ...
  useEffect(() => {
    ...
    void api.get<ProfileWithStats & { email?: string }>("/profile").then((d) => {
      ...
      setSocialLinks(
        d.socialLinks.map((l) => ({ platform: l.platform, url: l.url, label: l.label ?? "" })),
      );
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col gap-2">
              <label className="cursor-pointer">
                ...
                />
                <span className="inline-flex items-center gap-1 rounded-lg border border-[var(--color-border)] px-3 py-2 text-sm text-[var(--color-text)] hover:bg-[var(--color-background)]">
      ...
      <Card>
        <CardContent className="pt-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-[var(--color-text)]">
                {t("settings_blog_title")}
              </label>
              <Input
                value={form.blogTitle ?? ""}
                onChange={(e) => {
                  setForm((f) => ({ ...f, blogTitle: e.target.value }));
                }}
              />
            </div>
          </div>
          <div className="mt-4">
            <div className="mb-1 flex items-center justify-between">
              ...
              <div className="flex rounded-lg border border-[var(--color-border)]">
                <button
                  onClick={() => {
                    setAboutPreviewMode(false);
                  }}
                  className={`flex items-center gap-1 px-3 py-1 text-xs ${!aboutPreviewMode ? "bg-[var(--color-primary)] text-white" : "text-[var(--color-text-secondary)]"} rounded-l-lg`}
                >
                ...
                <button
                  onClick={async () => {
                    setAboutHtml(await parseMarkdown(form.aboutMe ?? ""));
                    setAboutPreviewMode(true);
                  }}
                  className={`flex items-center gap-1 px-3 py-1 text-xs ${aboutPreviewMode ? "bg-[var(--color-primary)] text-white" : "text-[var(--color-text-secondary)]"} rounded-r-lg`}
                >
        ...
        <CardContent className="pt-6">
          <div className="mb-4 flex items-center justify-between">
            ...
            <Button
              ...
              size="sm"
              onClick={() => {
                setSocialLinks((p) => [...p, { platform: "website", url: "", label: "" }]);
              }}
            >
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex justify-end">
              <Button onClick={handleAccountSave} disabled={accountSaving} variant="outline">
                ...
                {accountSaving ? t("loading") : t("settings_update_account")}
              </Button>
            </div>
          </div>
        </CardContent>
