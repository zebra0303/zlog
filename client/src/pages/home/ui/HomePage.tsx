...
export default function HomePage() {
  ...
  return (
    <div>
      <SEOHead title="홈" description="최신 게시글을 확인하세요." />
      <div className="mb-6 flex flex-wrap gap-2">
        <CategoryBadge slug="all" name="전체" isActive={!currentCategory} />
      ...
      {isLoading ? (
        ...
        <div className="py-20 text-center"><p className="text-lg text-[var(--color-text-secondary)]">게시글이 없습니다.</p><p className="mt-2 text-sm text-[var(--color-text-secondary)]">첫 번째 글을 작성해보세요!</p></div>
      )}
    </div>
  );
}
