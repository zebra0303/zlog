        ...
        <div className="flex flex-col items-center gap-4 sm:flex-row sm:items-start">
          ...
          <div className="flex-1 text-center sm:text-left">
            ...
            <div className="mt-4 flex justify-center gap-6 sm:justify-start">
              <div className="text-center"><span className="text-xl font-bold text-[var(--color-text)]">{profile.stats.totalPosts}</span><p className="text-xs text-[var(--color-text-secondary)]">{t("profile_posts")}</p></div>
              <div className="text-center"><span className="text-xl font-bold text-[var(--color-text)]">{profile.stats.totalCategories}</span><p className="text-xs text-[var(--color-text-secondary)]">{t("profile_categories")}</p></div>
              <div className="text-center"><span className="text-xl font-bold text-[var(--color-text)]">{profile.stats.totalViews}</span><p className="text-xs text-[var(--color-text-secondary)]">{t("profile_views")}</p></div>
            </div>
          </div>
          {isAuthenticated && <Button variant="outline" size="sm" asChild><Link to="/settings"><Settings className="mr-1 h-4 w-4"/>{t("profile_edit")}</Link></Button>}
        </div>
      </CardContent></Card>
      {profile.socialLinks.length > 0 && <Card><CardContent className="pt-6"><h2 className="mb-3 font-semibold text-[var(--color-text)]">{t("profile_social_links")}</h2><div className="flex flex-wrap gap-3">{profile.socialLinks.map((l) => <a key={l.id} href={l.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 rounded-lg border border-[var(--color-border)] px-3 py-2 text-sm text-[var(--color-text)] transition-colors hover:border-[var(--color-primary)] hover:text-[var(--color-primary)]">{ICONS[l.platform] ?? <LinkIcon className="h-5 w-5"/>}{l.label ?? l.platform}</a>)}</div></CardContent></Card>}
      {aboutHtml && <Card><CardContent className="pt-6"><h2 className="mb-3 font-semibold text-[var(--color-text)]">{t("profile_about")}</h2><div className="prose max-w-none dark:prose-invert" dangerouslySetInnerHTML={{ __html: aboutHtml }} /></CardContent></Card>}
    </div>
  );
}
