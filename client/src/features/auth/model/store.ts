...
export const useAuthStore = create<AuthState>((set) => ({
  owner: null, isAuthenticated: false, isLoading: true,
  login: async (email: string, password: string) => {
    const data = await api.post<LoginResponse>("/auth/login", { email, password });
    api.setToken(data.token);
    set({ owner: data.owner, isAuthenticated: true });
  },
  logout: () => { api.setToken(null); set({ owner: null, isAuthenticated: false }); },
  checkAuth: async () => {
    ...
    } catch { api.setToken(null); set({ owner: null, isAuthenticated: false, isLoading: false }); }
  },
}));
