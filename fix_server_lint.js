...
routeCode = routeCode.replace(
  /return c\.json\(\{ error: err\.message \}, (\d+)\);/g,
  'return c.json({ error: err instanceof Error ? err.message : "Error" }, $1);',
);
routeCode = routeCode.replace(
  /if \(err\.message === /g,
...
serviceCode = serviceCode.replace(
  /Array<\{ id: string; name: string }>/g,
  "{ id: string; name: string }[]",
);
serviceCode = serviceCode.replace(
  /targetCat\?.name \|\| "Unknown Category"/g,
  'targetCat?.name ?? "Unknown Category"',
);
